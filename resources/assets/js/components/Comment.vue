<template>
    <div class="pt-12 md:pl-10  text-2xl">
        <div class="bg-white  p-10 rounded mx-5" @mouseover="showReplyButton = true" @mouseleave="showReplyButton = false">
                <h6 class="text-2xl text-gray-400 underline">
                    {{ comment.user_name }}
                    <p class="md:inline md:pl-10 no-underline text-gray-500">{{ comment.created_at_humanized }}</p>
                </h6>
                <p class="text-gray-600 text-2xl pt-8">
                    {{ comment.body }}
                </p>
            ...
            <form class="flex md:pl-10" v-if="showReplyForm" @submit.prevent="addReply">
                <input 
                    type="text" 
...
export default {
    ...
    methods: {
        addReply() {
            ...
            axios({url: `/api/comments/${this.comment.id}/replies`, method: 'POST', data: formData })
            .then(response => {
        ...
        refreshReplies() {
            axios({url: `/api/comments/${this.comment.id}/replies`, method: 'GET'})
            .then(response => {
                let apiResponse = response.data

                this.comment.replies = apiResponse.data.replies.data
                this.stopSpinner()
            ...
            .catch(error => {
                ...
                alert("an error occured, kindly refresh the tab")
            })
        },
        resetForm() {
            this.userName = null 
